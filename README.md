...
| --- | --- | --- | --- |
| `--host` | `-h` | Reqable API host. | `127.0.0.1` |
| `--port` | `-p` | Reqable API port. | Uses `proxyPort` from local Reqable config when available, otherwise falls back to `9000` |

## Tool Overview

The server registers more than one hundred MCP tools. They are grouped by capability below.
